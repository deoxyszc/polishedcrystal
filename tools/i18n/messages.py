...
def select(lines,defines):
 ...
 for number,raw in enumerate(lines,1):
  ...
  if op=='if' and not macro:
   truth=(match[2] in defines) ^ bool(match[1]) if match else None
  ...
  known=conditional and (op=='endc' or all(s['truth'] is not None for s in stack))
...
def build(source,profile,old_catalog=None,old_diagnostics=None):
 ...
 for i,row in enumerate(rows[:-1]):
  if row['commands'] and row['commands'][-1]['op']=='stop_compressing_text':
   ...
   if following['source_path']==row['source_path']:
    ...
    row['translation_status']='shared_fallthrough_review';row['source_segments']+=following['source_segments']
 dispositions=[]
 for old in old_catalog or []:
  ...
  dispositions.append({'old_id':old['id'],'old_diagnostics':old['diagnostics'],'status':'inactive_profile_branch' if inactive else ('mapped' if matches else 'review_required'),'message_ids':[r['id'] for r in matches]})
 metadata=[]
...
def main():
 ...
 catalog.jsonl(a.out/'translation-ready.jsonl',[r for r in rows if r['translation_status'] in ('ready','ready_with_typed_format')])
