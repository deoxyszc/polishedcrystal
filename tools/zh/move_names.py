...
def generate(source,language,manifest):
 ...
 strip_map=json.loads((source/"data/zh/font/compiled.json").read_text())["move_grid"]
 ...
 for i,row in enumerate(rows,1):
  ...
  if width>MOVE_GRID.width*8:raise ValueError('Move name too wide: '+row['id'])
  lines.append(' dw .name'+str(i));bodies.extend(['.name'+str(i)+':',cache_text.compile_segments([dict(text=text)],glyphs,charmap,layout=MOVE_GRID,strip_map=strip_map)]);widths.append(width)
